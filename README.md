...
### Gestión Académica
- El código fuente vive dentro de `microservicios/microservicios_gestion` y comparte dependencias mediante su propio `requirements.txt`.
- Para ejecutarlo de forma local: `cd microservicios/microservicios_gestion`, creá un virtualenv (`python -m venv .venv`), instalá dependencias (`pip install -r requirements.txt`), copiá `.env.example` a `.env` y levantá `flask run --port 5002` o `python app.py`.
